...
module FalaAI
  class AuditoriaRiscoVerdictV2 < ApiModelBase
    ...
    # Risk matrix
    attr_accessor :risk_matrix

    # Applied actions
    attr_accessor :applied_actions

    # Decision details
    attr_accessor :decision_details
    ...
    def self.openapi_types
      {
        ...
        :'color' => :'String',
        :'icon' => :'String',
        :'risk_matrix' => :'Hash<String, Object>',
        :'applied_actions' => :'Array<AuditoriaRiscoAppliedActionV2>',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `FalaAI::AuditoriaRiscoVerdictV2` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FalaAI::AuditoriaRiscoVerdictV2`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'level_code')
        self.level_code = attributes[:'level_code']
      end
      ...
      if attributes.key?(:'risk_matrix')
        if (value = attributes[:'risk_matrix']).is_a?(Hash)
          self.risk_matrix = value
        end
      end

      if attributes.key?(:'applied_actions')
        if (value = attributes[:'applied_actions']).is_a?(Array)
          self.applied_actions = value
        end
      end

      if attributes.key?(:'decision_details')
        self.decision_details = attributes[:'decision_details']
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          color == o.color &&
          icon == o.icon &&
          risk_matrix == o.risk_matrix &&
          applied_actions == o.applied_actions &&
          decision_details == o.decision_details
    end
    ...
    def hash
      [label, level_code, color, icon, risk_matrix, applied_actions, decision_details].hash
    end
