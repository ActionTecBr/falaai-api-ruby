...
module FalaAI
  class AuditoriaRiscoAnalysisV2 < ApiModelBase
    # Global metrics
    attr_accessor :global_metrics

    # Final analysis
    attr_accessor :final_analysis
    ...
    def self.openapi_types
      {
        :'global_metrics' => :'Hash<String, Object>',
        :'final_analysis' => :'Hash<String, Object>',
        :'frameworks' => :'Hash<String, Object>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `FalaAI::AuditoriaRiscoAnalysisV2` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FalaAI::AuditoriaRiscoAnalysisV2`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'global_metrics')
        if (value = attributes[:'global_metrics']).is_a?(Hash)
          self.global_metrics = value
        end
      end

      if attributes.key?(:'final_analysis')
        if (value = attributes[:'final_analysis']).is_a?(Hash)
          self.final_analysis = value
        end
      end

      if attributes.key?(:'frameworks')
        if (value = attributes[:'frameworks']).is_a?(Hash)
    ...
    def ==(o)
      ...
      self.class == o.class &&
          global_metrics == o.global_metrics &&
    ...
    def hash
      [global_metrics, final_analysis, frameworks].hash
    end
