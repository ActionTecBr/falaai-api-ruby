...
module FalaAI
  class TranscriptionUsage < ApiModelBase
    # Actual audio duration processed in seconds
    attr_accessor :audio_seconds
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `FalaAI::TranscriptionUsage` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FalaAI::TranscriptionUsage`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'audio_seconds')
        self.audio_seconds = attributes[:'audio_seconds']
      else
        self.audio_seconds = nil
      end
    ...
    def list_invalid_properties
      ...
      if @audio_seconds.nil?
        invalid_properties.push('invalid value for "audio_seconds", audio_seconds cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @audio_seconds.nil?
    ...
    # @param [Object] audio_seconds Value to be assigned
    def audio_seconds=(audio_seconds)
      if audio_seconds.nil?
        fail ArgumentError, 'audio_seconds cannot be nil'
      end
    ...
    def hash
      [audio_seconds, credits_consumed, processing_ms].hash
    end
